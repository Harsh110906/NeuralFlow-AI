...
export function LogicNode({ data, selected }: { data: any, selected?: boolean }) {
  let borderColor = selected ? 'border-indigo-500 ring-4 ring-indigo-500/20' : 'border-purple-500';
  ...
  return (
    <div className={`px-4 py-2 shadow-md rounded-md bg-white border-2 min-w-[150px] transition-all ${borderColor}`}>
      <Handle id="target" type="target" position={Position.Top} className="w-3 h-3 bg-purple-500" />
