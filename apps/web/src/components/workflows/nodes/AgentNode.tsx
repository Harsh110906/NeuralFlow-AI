...
function AgentNodeComponent({ data, selected }: { data: any, selected?: boolean }) {
  ...
  return (
    <div className={`relative flex flex-col bg-white dark:bg-zinc-900 border ${borderColor} rounded-xl shadow-md transition-all duration-200 min-w-[200px] overflow-visible group`}>
      ...
      <div className={`absolute left-0 top-0 bottom-0 w-1.5 ${data.status === 'SKIPPED' ? 'bg-gray-400' : 'bg-blue-500'} rounded-l-xl opacity-80`} />
      ...
      <Handle 
        ...
        className={`w-3 h-3 bg-gray-200 dark:bg-zinc-700 border-2 border-white dark:border-zinc-900 -left-1.5 z-10 transition-colors ${data.status === 'SKIPPED' ? '' : 'hover:bg-blue-400'} hover:scale-125`} 
      />

      {statusOverlay}

      {/* Node Header */}
      <div className={`flex items-center gap-3 ${isCompact ? 'p-3' : 'px-4 py-3 border-b border-gray-100 dark:border-zinc-800'} ${data.status === 'SKIPPED' ? 'opacity-60' : ''}`}>
        <div className={`w-8 h-8 rounded-lg ${data.status === 'SKIPPED' ? 'bg-gray-100 text-gray-500 dark:bg-zinc-800' : 'bg-blue-50 dark:bg-blue-500/10 border border-blue-100 dark:border-blue-500/20 text-blue-600 dark:text-blue-400'} flex items-center justify-center shrink-0`}>
